...
<a href="#"><button type="button"  class="btn btn-xs btn-success btn-flat" id="'.$document_name.'" onclick="openDocument(this.id)"><span class= "glyphicon glyphicon-eye-open"> </span> view </button></a>
...
<div class="col-md- col-pull-right" style="text-align:right"><a class="btn btn-primary button1" href="#" id="<?php echo  $student_ID;?>"onclick="printMilestone(this.id)"  data-toggle="modal" data-target="#print_milestone_Modal"><i class="fa fa-print"></i><b> Print Milestone</b></a></div>
...
<form method="POST" action="view_children.php?id=<?php echo $student_ID;?>">
<div class="col-md-3">
...
echo $attendDate= date("Y-m-d", strtotime($attndDate));

$query22 = mysqli_query($conn,"select * from attendance where school_ID = '".$school_ID."'  and date_entered='".$attendDate."' and student_ID='".$student_ID."'  ");
while ($row11=mysqli_fetch_array($query22)){
$student_id=$row11['student_ID'];
$attendance_ID=$row11['attendance_ID'];
$query2 = mysqli_query($conn,"select * from student where school_ID = '".$school_ID."' and student_ID='".$student_id."'");
while ($row1=mysqli_fetch_array($query2)){
...
$attendDate= date("F d Y", strtotime($row11['date_entered']));

echo" <tr>

<td><input type='hidden' value='".$class_Id."' name='classId[]'>".$img." ".$name."</td>
<td>".$attendDate."</td>
...
// document.getElementById("dialog").innerHTML='<iframe src="../admin/document/'+document_name+'" style="width:100%;height:800px"></iframe>';

window.open("../admin/document/"+document_name, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=,left=,width=1000,height=1000");
}
